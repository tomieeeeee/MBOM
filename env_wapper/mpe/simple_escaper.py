...
class Simple_Escaper(object):
    def __init__(self, eps_max_step=30, env_version="simple_epcaper"):
        super(Simple_Escaper, self).__init__()
        self.n_agent = 2
        self.n_state = 27
        self.n_action = 9
        ...
        self.actions_trans = np.array([[1, 0, 0, 0, 0],
                                       ...
                                       [0, 0.3535, 0, 0.3535, 0], [0, 0, 0.3535, 0.3535, 0],
                                       [0, 0.3535, 0, 0, 0.3535], [0, 0, 0.3535, 0, 0.3535],
                                       ])
        self.env_state_running = False
        self.eps_max_step = eps_max_step
        self.cur_step = 0
        if env_version == "simple_epcaper":
            self.env = make_env('simple_escaper')
        else:
            raise NameError
